import cv2
import joblib
import numpy as np
import matplotlib.pyplot as plt
from pathlib import Path
from skimage.feature import hog
from tqdm import tqdm

# This script is used to plot DET curves for models using different block size:8x8,16x16,32x32

win_w, win_h = 64, 128
stride = 16

def sliding_windows(img):
    """
    Generator function to yield patches using a sliding window over the input image.
    Each patch is of size (win_h, win_w) and moves with the defined stride.
    """
    h, w = img.shape
    y_positions = list(range(0, h - win_h + 1, stride))
    if (h - win_h) % stride != 0:
        y_positions.append(h - win_h)
    x_positions = list(range(0, w - win_w + 1, stride))
    if (w - win_w) % stride != 0:
        x_positions.append(w - win_w)
    for y in y_positions:
        for x in x_positions:
            yield img[y:y + win_h, x:x + win_w]

def list_images(folder, exts={'.jpg', '.jpeg', '.png', '.bmp'}):
    # Recursively list all image files in a folder with specified extensions.
    return [p for p in Path(folder).rglob("*") if p.suffix.lower() in exts]

def evaluate_det_curve(model_path, hog_params, thresholds):
    # Evaluate the DET curve for a given model and set of HOG parameters.
    base = Path(__file__).resolve().parent.parent
    model = joblib.load(model_path)

    human_imgs = list_images(base / "dataset-big/Testing set/human")
    nonhuman_imgs = list_images(base / "dataset-big/Testing set/non-human")
    results = []
    #Compare true positive and false positive rates at various thresholds
    for threshold in thresholds:
        miss = 0
        total_fp = 0
        total_windows = 0
        true_positive = 0
        pbar = tqdm(total=len(human_imgs) + len(nonhuman_imgs), desc=f"Threshold {threshold:.2f}")
        
        # Evaluate on human images
        for path in human_imgs:
            img = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE)
            if img is None:
                pbar.update(1)
                continue
            h, w = img.shape
            # If image is smaller than window, resize it      
            if h < win_h or w < win_w:
                img = cv2.resize(img, (win_w, win_h))
                f = hog(img, **hog_params)
                if model.decision_function([f])[0] > threshold:
                    true_positive += 1
                else:
                    miss += 1
                pbar.update(1)
                continue

            detected = False
            # Slide window over the image, check if any patch triggers a positive detection
            for patch in sliding_windows(img):
                f = hog(patch, **hog_params)
                if model.decision_function([f])[0] > threshold:
                    detected = True
                    break
            if detected:
                true_positive += 1
            else:
                miss += 1
            pbar.update(1)
        # Evaluate on non-human images
        for path in nonhuman_imgs:
            img = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE)
            if img is None:
                pbar.update(1)
                continue
            h, w = img.shape
            # If image is smaller than window, resize it
            if h < win_h or w < win_w:
                img = cv2.resize(img, (win_w, win_h))
                f = hog(img, **hog_params)
                total_windows += 1
                if model.decision_function([f])[0] > threshold:
                    total_fp += 1
                pbar.update(1)
                continue
            # Slide window, count all false positives
            for patch in sliding_windows(img):
                f = hog(patch, **hog_params)
                total_windows += 1
                if model.decision_function([f])[0] > threshold:
                    total_fp += 1
            pbar.update(1)
        # Calculate miss rate and FPPW
        miss_rate = miss / len(human_imgs)
        fppw = total_fp / total_windows
        results.append((threshold, fppw, miss_rate))
        pbar.close()

    return results

def plot_det(det_curves, save_path):
    # Plot the DET curves for different model configurations.
    plt.figure(figsize=(8, 6))
    for label, points in det_curves.items():
        fppw = [x[1] for x in points]
        miss = [x[2] for x in points]
        plt.plot(fppw, miss, label=label, marker='o')
    plt.xscale("log")
    plt.yscale("log")
    plt.xlabel("False Positives Per Window (log scale)")
    plt.ylabel("Miss Rate (log scale)")
    plt.title("DET Curve: Effect of Block Size")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(save_path)
    print(f"[Saved] DET curve at {save_path}")

if __name__ == "__main__":
    # Define thresholds to sweep over for DET evaluation
    thresholds = np.linspace(-1.5, 1.5, 11)

    base = Path(__file__).resolve().parent.parent
    # Paths for different models with different block size
    model_b8 = base / "models/hog_svm_model_v2_8.pkl"
    model_b16 = base / "models/hog_svm_model_v2.pkl"
    model_b32 = base / "models/hog_svm_model_v2_32.pkl"

    # Define HOG parameters for each block size setting
    hog_block8 = {
        'orientations': 9,
        'pixels_per_cell': (8, 8),
        'cells_per_block': (1, 1),  # 8x8 block
        'block_norm': 'L2-Hys',
    }
    
    hog_block16 = {
        'orientations': 9,
        'pixels_per_cell': (8, 8),
        'cells_per_block': (2, 2),  # 16x16 block
        'block_norm': 'L2-Hys',
    }

    hog_block32 = {
        'orientations': 9,
        'pixels_per_cell': (8, 8),
        'cells_per_block': (4, 4),  # 32x32 block
        'block_norm': 'L2-Hys',
    }

    print("[INFO] Evaluating block=8 model...")
    res8 = evaluate_det_curve(model_b8, hog_block8, thresholds)

    print("[INFO] Evaluating block=16 model...")
    res16 = evaluate_det_curve(model_b16, hog_block16, thresholds)
    
    print("[INFO] Evaluating block=32 model...")
    res32 = evaluate_det_curve(model_b32, hog_block32, thresholds)

    # Plot and save the DET curves
    curves = {
        "block=8": res8,
        "block=16": res16,
        "block=32": res32
    }
    plot_det(curves, base / "outputs/det_blocksize.png")