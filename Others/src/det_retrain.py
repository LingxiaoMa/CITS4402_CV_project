...
def plot_det(det_curves, save_path):
    ...
    plt.title("DET Curve: Effect of Retrain")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    base = Path(__file__).resolve().parent.parent
    # Paths for models before and after retrain
    model_v1 = base / "models/hog_svm_model_v1.pkl"
    model_v2 = base / "models/hog_svm_model_v2.pkl"


    # Define HOG parameters for models
    hog_block16 = {
        'orientations': 9,
        'pixels_per_cell': (8, 8),
    ...
    print("[INFO] Evaluating v1 model...")
    res_v1 = evaluate_det_curve(model_v1, hog_block16, thresholds)

    print("[INFO] Evaluating v2 model...")
    res_v2 = evaluate_det_curve(model_v2, hog_block16, thresholds)
    


    curves = {
        "model before retrain": res_v1,
        "model after retrain": res_v2,
    }
    # Plot and save the DET curves
    plot_det(curves, base / "outputs/det_retrain.png")
