...
def list_images(dir_path, valid_exts={".jpg", ".jpeg", ".png", ".bmp"}):
    # Recursively list all image files in a directory with valid extensions.
    return [p for p in Path(dir_path).rglob("*") if p.suffix.lower() in valid_exts]

def extract_features(image, label, size=(64,128)):
    # Extract HOG features from a list of images and return features with their corresponding labels.
    features = []
    labels = []
    for path in tqdm(image, desc=f"Extracting HOG for label {label}"):
        img = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE) # Load image in grayscale
        if img is None:
            continue
        img = cv2.resize(img, size) # Resize to target size
        hog_feat = hog(img, **HOG_PARAMS) # Extract HOG features
